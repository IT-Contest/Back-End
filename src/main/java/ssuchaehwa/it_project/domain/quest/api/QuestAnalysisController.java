...
public class QuestAnalysisController {
    ...
    @GetMapping("/daily")
    @Operation(summary = "일일 퀘스트 분석 API", description = "사용자의 최근 7일간 일일 퀘스트 완료 현황을 분석합니다.")
    @ApiResponses({
    ...
    public BaseResponse<List<AnalysisResponseDTO.Daily>> getDailyAnalysis(
        ...
        List<AnalysisResponseDTO.Daily> rows = questAnalysisService.getDaily(userId);
        return QuestConverter.toDailyAnalysisResponse(rows);
    }

    @GetMapping("/weekly")
    @Operation(summary = "주간 퀘스트 분석 API", description = "사용자의 이번 달 주차별 퀘스트 완료 현황을 분석합니다.")
    @ApiResponses({
    ...
    public BaseResponse<List<AnalysisResponseDTO.Weekly>> getWeeklyAnalysis(
        ...
        Long userId = principal.getId();
        List<AnalysisResponseDTO.Weekly> rows = questAnalysisService.getWeekly(userId);
        return QuestConverter.toWeeklyAnalysisResponse(rows);
    }

    @GetMapping("/monthly")
    @Operation(summary = "월간 퀘스트 분석 API", description = "사용자의 이번 달 포함 전월 12개월 퀘스트 완료 현황을 분석합니다.")
    public BaseResponse<List<AnalysisResponseDTO.Monthly>> getMonthlyAnalysis(
            @AuthenticationPrincipal UserPrincipal principal
    ) {
        Long userId = principal.getId();
        var rows = questAnalysisService.getMonthly(userId);
        return QuestConverter.toMonthlyAnalysisResponse(rows);
    }

    @GetMapping("/yearly")
    @Operation(summary = "연간 퀘스트 분석 API", description = "사용자의 이번 년도 포함 전년 10개년 퀘스트 완료 현황을 분석합니다.")
    public BaseResponse<List<AnalysisResponseDTO.Yearly>> getYearlyAnalysis(
            @AuthenticationPrincipal UserPrincipal principal
    ) {
        Long userId = principal.getId();
        var rows = questAnalysisService.getYearly(userId);
